- [HITRAN Data](https://hitran.org/data-index/) - Full absorption spectra for all gases
	- [Definitions and Units](https://hitran.org/docs/definitions-and-units/)
- Planck's Law ($W / m^3$)
	...
	- Multiply by $\displaystyle \left( \frac{r_{star}}{r_{orbit}} \right)^2$ to get planetary intensity
- Lorentzian Line Profile
	...
	- $\displaystyle \alpha(\nu) = \frac{S L(\nu, \nu_0, \gamma)}{1 + \Delta P}$
		- $\displaystyle L(\nu, \nu_0, \gamma) = \frac{1}{\pi}\frac{\gamma}{\left(\nu - \nu_0\right)^2+\gamma^2}$
		- $\gamma = \gamma_{air} + \gamma_{self}$
		- $\displaystyle\gamma_{air}= \gamma_{air,0} \left(\frac{P-P_{self}}{P_0}\right)\left(\frac{T_0}{T}\right)^{n_{air}}$
		- $\displaystyle\gamma_{self}=\gamma_{self,0}\left(\frac{P_{self}}{P_0}\right)\left(\frac{T_0}{T}\right)^{n_{self}}$
	-
	-
-